...
#            fc_2 = bn(fc_2, self.is_trainingtf)
        ...
        with tf.variable_scope("FC_layer3") as scope:            
            fc_out_real = tf.layers.dense(fc_2, units = 257, activation = tf.nn.sigmoid, trainable = self.is_training, kernel_initializer = tf.truncated_normal_initializer(stddev=0.01))
            fc_out_imag = tf.layers.dense(fc_2, units = 257, activation = tf.nn.sigmoid, trainable = self.is_training, kernel_initializer = tf.truncated_normal_initializer(stddev=0.01))
            return fc_out_real, fc_out_imag
########################################################################
